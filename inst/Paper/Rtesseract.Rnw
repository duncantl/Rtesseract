...
It seems not. Of course, there is text under this highlight.
...
(i.e., internally Tesseract operates off the \code{Pix} object created by
...
We do this with the  \code{pixConvertTo8()} function.
...
We can then pass the modified image  to the \code{tesseract()} function:
