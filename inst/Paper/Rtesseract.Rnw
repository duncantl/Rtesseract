...
read back into \proglang{R}. Then, following the creation of \pkg{Rtesseract},
...
are accessible through \proglang{R} via the \pkg{Rtesseract} package, including
...
development version of Tesseract (4.00+), which is currently available from
