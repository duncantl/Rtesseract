...
names is a mistake, but we might not be sure what the correct letter
should be.
...
Again, we can see the text is rotated for this character, resulting in
poor OCR results.
...
in the \code{...}  arguments for any of the high-level functions,
...
%% Too hacky - fix this in the package
<<>>=
grep("^PSM", ls("package:Rtesseract"), value = TRUE)
@ 
...
Memory (LSTM) neural network-based OCR engine is included.
...
or in the call a base level function,

<<>>=
GetText(smithburn, engineMode = "OEM_CUBE")
@ 
