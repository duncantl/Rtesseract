...
and \code{GetBoundingBoxes}. Similar to \code{GetText}, these
...
the user to create a persentent \code{TesseractBaseAPI} object using
...
once the image has been OCRed, subsequent queries for additional
...
interested to know how the shadow on the image affected the OCR
performance. Since the confidences for each match are available to us,
...
scatter.smooth(bb[,5] ~ rowMeans(bb[,c(1,3)]),
     xlab = "Position of bounding box on page",
...
Similarly, by accessing the information regarding the bounding boxes,
users are able to use location or size specific information manipulate
the results of the OCR. In the above image, identifying the title from
plain text would involve using context-based heuristics, but with the
text size in hand the matter becomes far simplier:
