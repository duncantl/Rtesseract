...
the OCR will be performed (explained in more detail in ``Advanced
Use''). We then pass this to each of the four high level functions
...
the bounding box around the word, symbol, textline, etc.
...
at the image, we can see there is rotated text in the margin which is
not being recognized well by the OCR. Because of this text crosses
several lines, post-processing without location information would
involve searching for ``junk'' at the end of each text lines. However,
we can easily exclude this text with location information in
hand. Additionally, while we could crop this image using editing
software outside of R, later we will show how this area can be
excluded from the OCR using \pkg{Rtesseract} without leaving R.

To continue, suppose we also wanted to extract the author's name, knowing
that this information is present at the top of each page,
