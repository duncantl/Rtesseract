...
Note that the background is black in the \code{Pix} object, corresponding to values of 0.
Adding the option \code{invert = TRUE} will invert the \code{Pix}.
We see the 4 primary lines that span most of the width of the
