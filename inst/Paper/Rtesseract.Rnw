...
\Abstract{
  ...
  \pkg{Rtesseract} allows an \proglang{R} user to recover of text from an image
...
inspect the alternative matches, in this case for the sixth character ``c'',
...
\subsection{Zooming and subsetting an image using \pkg{Rtesseract}}

It is sometimes useful to ``zoom in'' on a part of the image to inspect a region that is not being accurately recognized.  For visualizing a sub-area of the original image, we use the \code{plotSubImage()} function. For example, if we wanted to zoom in on the lowest confidence word,
...
However, there are times when there is only a part of the image that we want to recognize with OCR.
...
Then we reduce the recognization rectangle to only this area, with the diminsions specified as left edge, top edge, width of box, height of box,
...
%To avoid messing something up later
<<echo = FALSE, results = "hide">>
SetRectangle(api, dims = c(0,0,4050, 6000))
@ 
...
First, two ``meta-variables'' control which OCR engine is used,
...
\subsection{Image Manipulation and Pre-processing using \pkg{Rtesseract}}
