...
% Fix this
<<echo = FALSE>>=
f = system.file("images", "SMITHBURN_1952_p3.tiff", package = "Rtesseract")
@ 
...
<<>>=
#f = system.file("images", "Biological_YF_Risk-0_01.png", package = "Rtesseract")
f = system.file("images", "SMITHBURN_1952_p3.tiff", package = "Rtesseract")
...
However, merely retrieving text can be of limited usefulness. We would
like to check the results, but checking by eye, word by word is both
tedious and error-prone. Rather, we would like focus our attention
initially at those words with the lowest confidences:

<<>>=
confs = GetConfidences(f)
sort(confs)[1:10]
@ 

Clearly, some of these do not look correct. To further investigate
these, we might want to see where they occur on the page:
...
It appears the low confidence words might be grouped together in
similar areas on the page. Since we have access to the confidences in
R, we can treat these just like any other data. For example, we can
group the words into 10 bins by their location left to right,
...
Clearly there are issues with the right side of the page. Looking
at the image, we can see there is text in the margin at a different
orientation which is not being recognized well by the OCR.

This example highlights both the workflow of using OCR to extract
text and why having access to information beyond just the text is so
crucial.
...
the OCR will be performed. We then pass this to each of the four high
level functions rather than the file name, e.g., \code{GetText(api)},
...
For example, say we wanted explore the results of OCR on the image
above. 

to OCR the previous image, but also were
...
statistically and/or visually. For example, we have a print out of an
old dot-matrix print out of data that we would like to add to an
analysis in R,
...
\pkg{Rtesseract} includes a plotting method for OCR
...
confidence (see Fig. \ref{fig:rtess_plot} for an example). We have
found these plots to helpful in diagnosing OCR behavior and
inaccuracies. Plotting the results is especially helpful when
adjusting the OCR configuration settings (discussed below under
``Advanced Use'').
