...
% e.g.
  % We are increasingly recovering text and numeric data from scanned documents
  % and images.  The \pkg{Rtesseract} provides flexible and comprehensive functionality for
  % this using optical character recognition (OCR) and is an interface to the Open Source C++
  ...
  provides access to Tesseract, an open source \proglang{C++} library for Optical
  ...
  as lines, individual words or separate characters.  Importantly, we can also obtain the
  location of text elements, possible alternatives for characters, and the estimated
  ``confidence'' for each.  These metadata are often essential for interpreting
  and reconstructing the text, e.g., for multi-column layout, tables, or even section titles.  The package provides access to much of the Tesseract C++ API, allowing full
  customization of the OCR behavior, and exporting the results in
  various formats (e.g. PDF), and also visualizing them within R
  itself.  The package also provides access to leptonica's image manipulation
  functionality for pre-processing images to enhance the recognition, and also
  high-level functionality for recovering horizontal and vertical lines often vital
...
%FIX: citations for each of these
currently available (e.g., Adobe Acrobat, PDFPen Pro, ABBY, Google Cloud Vision), 
though these
typically  %FIX: this word is tricky. ABBY outputs XML with information.
  % Perhaps  "however not all of these...."
  % And will the reader understand the implications of this at this point in the text.
only output text results.
...
proceeding. Therefore, OCR is commonly part of a workflow that involves
...
etc. using image editing software to make the text ``stand out'' prior to OCR. Then
after OCR has been run, we will check if the results make sense. In
some cases there are obvious issues, such as nonsense words or garbled %FIX what does garbled mean
...
the text results using methods such as those used in Natural Language Processing (NLP). 
% New paragraph possibly??
Using OCR software which only returns text results, our full OCR workflow would involve 1)
...
4) no access to OCR intermediate metadata, such as confidence measures,
alternative matches, location on the page, size of the text, etc., which are helpful for
assessing the certainty in each returned result and recovering structure and layout
of the data such as columns in a table or section titles.
%FIX: Perhaps make this punchier, stronger and more succinct. Not certain the
% reader will get the need for this metadata

In our work extracting and analyzing text data present in images from
\proglang{R}, we desired  % required? needed?
a tool which would overcome the limitations above. % perhaps be specific about which 
...
how the OCR is done.
%FIX: What about recovering lines, pre-processing the image (rotation), 
% zooming in on specific regions for more focused OCR (perhaps based on the metadata from the original OCR)
...
actively maintained and developed by the open source community % FIX: the word "the" 
  % for open source community may not be ideal as there are many more than one 
(including developers at Google). Recent innovations in Tesseract
include the integration of LSTM Deep Learning networks (versions $\ge$
...
on a sub-region of the image, rotate the image, threshold the colors, or enhance the image with some computer
...
fonts. We can adjust the OCR behavior via run-time configuration variables to
...
the high-level structure from the individual data values. If the
...
Similar bindings to the Tesseract API exist in \proglang{Python}, \proglang{Ruby}, \proglang{Go}, etc. (see \url{https://github.com/tesseract-ocr/tesseract/wiki/AddOns} for additional details on these interfaces to Tesseract), though these typically only provide limited functionality.
...
image.  \pkg{Rtesseract} additionally
allows \proglang{R} users access to leptonica\citep{leptonica}, the image library Tesseract uses for
image pre-processing. This functionality allows users to provide their own
processed image to Tesseract. Lastly, \pkg{Rtesseract} has been
designed to support both the current release (v3.05.01) and
development versions (v4.0.0) of Tesseract, with mechanisms to
...
default, it returns the words it finds in the image in the order they % order needs qualification or some definition
...
Possible values include ``block'', ``para'' (paragraph), ``textline'' and ``word''.
...
Then we reduce the recognization rectangle to only the area below this word using the \code{SetRectangle()}function,
...
image. In many non-trivial cases, we might want to manipulate the
...
or in the call to a base level function when calling on a image filename,
...
off a \code{Pix} object (Leptonica's image object format) created by Leptonica after this
