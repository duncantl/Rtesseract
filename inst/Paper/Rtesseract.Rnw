...
from older PDF documents that are not ``text searchable.'' There are
many pieces of software that provide a means to convert images of
data into computer-usable formats (e.g., MS Word, Acrobat, PDFPen Pro,
etc.), but these typically (1) lack fine scale control and/or ability
to customize training and (2) are separate from the tools being used
to analyze the resulting data. Ideally, there would be a means to
extract information from images or PDF documents directly into the
environment where analysis is being done to eliminate the need for
multi-software workflows.

Here, we describe a low-level and highly expandible tool to extract
text and data from images into R, an environment for statistical
computing. This tool uses Optical Character Recognition (OCR), a
method to extract strings or characters from an image that can then be
directly manipulated by a program. Typically OCR is used to convert a
...
``searchable'' PDF document, etc.. In addition to extracting numeric
data, OCR allows the extraction of text, expanding corpus available
for the increasingly powerful capacity to analyze the contents of text
