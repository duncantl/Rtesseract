...
the fine-scale controls for more advanced use cases.
Since we initially created \pkg{Rtesseract} in April 2015, 
two other R packages interfacing to Tesseract have been released.
The \pkg{ocR} package~\citep{Hagmann2014} provides
an interface to calling the Tesseract command line executible.
Using the \pkg{ocR} package, the user specifies the image file and read the results that Tesseract writes to a file.
The \pkg{tesseract} package~\citep{ooms2016}
takes a similar approach to \pkg{Rtesseract} package 
by interfacing to the C++ library.
However, by design, it provides only functionality to 
get the text results from the OCR process.
\pkg{Rtesseract}, from its initial implementation,  provides 
access to a most of the functionality in te Tesseract API
and provides fine-grain control from R to the different steps in the OCR process.
\pkg{Rtesseract} supports both the current release (v3.05.01) and
