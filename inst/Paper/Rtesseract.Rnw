...
<<>>=
longLines = rowSums(pMat) > 1000
@

The value 1000 (approximately 25\% of the width of the page) is an initial guess.
This includes both lines that span 1000 pixels or more and lines that have several smaller segments whose length adds up to 1000 pixels.
This might be too large to detect the shorter line segments
...
We can visualize these 46 horizontal lines on the image with
...
Note again that the rows in the matrix go from 1 to \code{nrow}, while on the
plot, they go down the page.  This is why we get the height with
\code{nrow(pMat) - which(longLines)}.
...
The gray marks on the vertical lines have disappeared in Fig \ref{fig:remove_smudge}.
...
This creates a separate PDF file named vhlinefinding.pdf.
