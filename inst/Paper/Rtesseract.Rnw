...
supported format (tiff, JPEG, and png on most systems), and to perform OCR
on the image using the default settings. For example, suppose we have
...
We sometimes want the results returned as individual characters (symbols), % Motivate why
% For grammar reasons (i.e. the "All of which" below), we probably want to rephrase this.
...
% All here is capitalized yet part of sentence.
...
obtain a different output from the OCR engine. Since OCR is
...
error-prone. Rather, we might want to focus our attention initially on
