...
in the Tesseract API and provides fine-grain control from R to the
...
an image of the first page of a scientific publication which we wanted
to extract information from (Fig. \ref{fig:base}). We will use this
image to highlight the functionality and common use-cases of the
\pkg{Rtesseract} package.
...
However, rather than words, we sometimes want the results returned as
individual characters (symbols),
...
might wish to know the location of the match,

<<>>=
GetBoxes(f)
@ 

how confident the OCR engine is in each match,

<<>>=
GetConfidences(f)
@

and what alternatives exist for the returned match,
...
\code{GetBoxes} for the same file is a common occurrence. Yet, it has a
distinct inefficiency - we repeat the OCR recognition the same file
merely to record a different product from the OCR engine. Since OCR is
...
This process of creating a persistent object which can be repeatedly
queried is the preferred method when multiple piece of information are
...
level functions rather than the file name,
...
e.g., calling any of the functions above, or explicitly calling
...
Furthermore, \pkg{Rtesseract} includes a plotting method for OCR
