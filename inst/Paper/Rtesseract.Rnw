...
\Abstract{
  ...
  data present in images. \pkg{Rtesseract} is an R package that
  provides access to Tesseract, an Open Source C++ library for Optical
  Character Recognition (OCR), from within R.  At its simplest,
  \pkg{Rtesseract} allows an R user to recover of text from an image
  ...
  a fully programmatic workflow to extract text data into R for
  subsequent analysis.  }
...
documents such as word-processing files, HTML, PDF. However, there are
...
accessible for data analysis. There are many OCR software tools
available (e.g., Adobe Acrobat, PDFPen Pro, ABBY), though these
typically only output text results to a file.
...
R, we desired a tool which would overcome the limitations above. In
...
active development. Accessing the Tesseract API from R provides us
with the ability to bring pre-processing, OCR, post-processing, and
analysis under complete programmatic control within R. The
...
read back into R. Then, following the creation of \pkg{Rtesseract},
...
Tesseract API and provides fine-grain control from R to the different
...
development versions (v4.00.alpha) of Tesseract, with mechanisms to
configure \pkg{Rtesseract} according to the installed version of
Tesseract.
...
(an object of class \code{TesseractBaseAPI} in R).  We can repeatedly
...
showing up often in the lowest cofidence results.

Since we have access to the confidences and bounding boxes in R, we
...
We know that the lower case letter in ``c.'' is a mistake, but we
might not be sure what the correct letter should be. Helpfully, we can
...
settings to improve accuracy (discussed further below under ``Advanced
Use'').
...
First off, two ``meta-variables'' control which OCR engine is used,
and how that engine recognizes the arrangement of text,
\code{pageSegMode} and \code{engineMode}. Since adjusting these two
variables can have large impact on the results of the OCR, they
deserve special mention. The first, the Page Segmentation Mode,
adjusts how Tesseract recognizes lines of text in the image. By
adjusting Page Segmentation Mode, the user can tell Tesseract how to
expect the text to be arranged, e.g. as a single block of text, sparse
text, arranged top to bottom (e.g., the verticle arrangement of many
Asian texts) or even in a circle (e.g., the text on the rim of a
coin), etc. Included in the available options are to have Tesseact
attempt to automatically detect the text orientation, the script used,
or both. %more here The \pkg{Rtesseract} package preserves the
defaults in the Tesseract C++ API and assumes the input is a single
...
ways: in the call to \code{tesseract}, in the \code{...}  arguments
for any of the high-level functions (e.g., \code{GetText(smithburn,
  pageSegMode = "PSM\_AUTO")}), or by using \code{SetPageSegMode} on
an existing \code{TesseractBaseAPI} object.

The second of these variables is the Tesseract Engine Mode, which
determines which OCR engine is used to recognize text. Tesseract ships
with additional OCR algorithms which can be used individually or in
combination with Tesseract, though by default \pkg{Rtesseract} uses
Tesseract only. For Tesseract versions 3.05+, an additional OCR engine
Cube is included, and for Tesseract versions 4.00+ a Long Short-term
Memory (LSTM) neural network based OCR engine is included. Although
the use of each of these in combination with Tesseract can potentially
...
Consider the previous image. When we perform OCR on this, tesseract
...
Some of the words in the highlighted text are ``Drs. Dick and
...
even R itself operating on the image as a matrix or array. Leptonica
...
A primary motivation underlying the Rtesseract package is that we need context from the rendered page to make sense of the results of the OCR.
...
interpret the content.   This includes identifying document and section titles in the document based on the size of the text.  In this example, we'll look at the locations of lines on the page in order to be able to interpret data within tables.   Again, consider the previous image.
...
(Note that this creates a new Pix and allows the previous one to be
garbage collected as we assign the new image to the same R variable.)
...
colored white.  These occur in the corners.   We can specify any
...
and then desribe a "mask" or box that helps to identify contiguous
regions in the image corresponding to the lines we want.  For
...
of the image as R plots from bottom up and the coordinates for the
...
We specify the Pix to process and then the horizontal width and
...
within the region are not white.   findLines() uses the function
pixCloseGray() to do this.
...
The computation is a little akward.  We split the indices of rowNums
...
Lets consider the first element of \code{ll}.  It has 14 rows and 4050
...
black.)  So there are many white pixels eventhough the black pixels
...
for more advanced uses.  By allowing recovery of additional
...
inputting data from images into R (e.g., spatial analysis of text).
Leveraging free, open-source software allows full integration of the
work-flow of text from images into R.
