...
\caption{\pkg{Rtesseract}'s plotting function allows the user to
  overlay the original image with the bounding boxes surrounding the
...
lines, etc. and since Tesseract conducts OCR off the \code{Pix} object created by
Leptonica, we include some bindings to the Leptonica library in the
\pkg{Rtesseract} package for basic image processing. Using these
functions, we can conduct our own pre-processing and then hand the
resulting \code{Pix} object to Tesseract for OCR.
...
z = which(rr >= 4.5)
plot(h, invert = TRUE)
points(z, nrow(m) - rep(as.integer(names(ll)[2]), length(z)), col = "green", pch = 16, cex = 0.5)
@
\caption{}
\label{}
\end{figure}
