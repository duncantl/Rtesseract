...
Using the bounding boxes or other coordinates as guides, we are able
to adjust the OCRed area without leaving R. Typically, to do this
would involve partitioning the image into subsets using an image
editing tool (e.g., Gimp, Darktable, Photoshop), and then running the
OCR on each of these subset images. However, \pkg{Rtesseract} allows
the user to specify a rectangle of interest using the
\code{SetRectangle} function, which takes the left, top, width and
height of the sub-area as additional arguments. Once set, the next
call to the \code{TesseractBaseAPI} will only OCR within this
rectangle - no image manipulation required. Continuing the example,
say we were primarily interested in the abstract. We know that the
